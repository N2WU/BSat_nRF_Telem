...
//****************************************************************************
char  CallSign[7] = "W2KGY"; //DO NOT FORGET TO CHANGE YOUR CALLSIGN
...
void updateTelemetry() {
  ...
  char text = readRadio();
  pos = text.find(":");    // position of ":" in str (format is DATA (unit): NUM)
  telem_num = str.substr (pos);   // get from ":" to the end
  pos1 = text.find("(");    // position of ":" in str (format is DATA (unit): NUM)
  pos2 = text.find(")");    // position of ":" in str (format is DATA (unit): NUM)
  telem_unit = str.substr(pos1, pos2-pos1);   // get inbetween ()
  
  sprintf(telemetry_buff + 54, "%03d", telem_num);
  sprintf(telemetry_buff + 56, "%s", telem_unit);
